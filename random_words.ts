...
function pickRandomWord(filename: string): string {
  // Read the file contents
  const data = fs.readFileSync(filename, 'utf-8');

  // Split the data into lines (assuming each line is a word)
  const allWords = data.split(/\r?\n/); // Handles both LF (\n) and CRLF (\r\n) line endings

  // Generate a random index within the list bounds
  const randomIndex = Math.floor(Math.random() * allWords.length);

  // Return the word at the random index with trailing newline removed
  return allWords[randomIndex].trim();
}
